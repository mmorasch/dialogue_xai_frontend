...
<div>
	...
	<Stepper {currentStep} stepTerm="Questions" on:step={onNext} on:complete={onComplete}>
		...
		{#each chunks as chunk, chunkIndex}
			<Step>
				<h1 class="center-text text-xl">
					Please answer the following questions and give us feedback to improve.
				</h1>
				{#each chunk as question, i}
					<div class="flex">
						...
						<div class="slider-column">
							<RangeSlider
								name="range-slider"
								bind:value={answers[chunkIndex * 7 + i]}
								min={-3}
								max={3}
								step={1}
								ticked
		...
		<Step>
			...
			<h2>
				Since this is the prestudy, please leave any opinion and your name here so we can talk and I
				can link the experiment results to you for further analysis together. :)
			</h2>
			<textarea class="feedback-textarea" bind:value={feedback} />
			<!-- Bind the value here -->
		</Step>
	</Stepper>
</div>
